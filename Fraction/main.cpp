...
{
	const int SIZE = 256; 
	char buffer[SIZE] = {};		// строку сохраняем в буфер 
	//is >> buffer; 
	is.getline(buffer, SIZE);
	int number[3] = {};		// здесь храняться числа, введенный с клавиатуры 
	int n = 0;				//  счетчик чисел. посчитать сколько ввели чисел 
	const char delimiters[] = "( /)"; // для функции strtok
	for (char* pch = strtok(buffer, delimiters); pch; pch = strtok(NULL, delimiters))// вытаскиваем Токены - полезная инфа
	{
		// указатель pch хранит адрес начала токена
		// в функцию strtok() только первыйраз передается делимая строка
		...
		number[n++] = atoi(pch);
		// atoi - ASCII - string To Integer (функция преобразует строку в значение типа int)
	}
